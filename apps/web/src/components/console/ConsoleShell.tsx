...
/** Demo or live, always visible in the console; admins can click through to the switch. */
function ModeChip({ mode, admin }: { mode: 'demo' | 'live'; admin: boolean }) {
  const chip = <Badge tone={mode === 'demo' ? 'amber' : 'verify'}>{mode === 'demo' ? 'Demo' : 'Live'}</Badge>;
  return admin ? <Link href="/admin/site" title="Site mode" className="inline-flex">{chip}</Link> : chip;
}

export async function ConsoleShell({ realm, nav, session, children }: { realm: string; nav: { href: string; label: string }[]; session: Session; children: React.ReactNode }) {
  const mode = await siteMode(); const admin = session.role === 'admin';
  return (
    <div className="flex min-h-dvh bg-parchment/50">
      <aside className="hidden w-64 shrink-0 flex-col border-r hair bg-paper lg:flex">
        <div className="border-b hair px-5 py-4"><Wordmark /><div className="mt-3 flex items-center gap-2"><Badge tone="ink">{realm}</Badge><ModeChip mode={mode} admin={admin} /></div></div>
      ...
      <div className="min-w-0 flex-1">
        <header className="flex h-14 items-center justify-between border-b hair bg-paper px-5 lg:hidden"><Wordmark /><span className="flex items-center gap-2"><Badge tone="ink">{realm}</Badge><ModeChip mode={mode} admin={admin} /></span></header>
