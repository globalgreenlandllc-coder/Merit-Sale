...
export default async function AdminDashboard() {
  ...
  const rows = await Promise.all(opens.map(async (o) => {
    ...
    return { o, c, r1Attempts, r2Attempts };
  }));
  ...
  return (
    <>
      <PageHead label="Platform admin" title="Dashboard" />
      ...
      <Card className="mt-8" title="Funnel per Merit Open · reservation → registration → R1 → R2 (no score access in this realm)">
        <Table head={['Merit Open', 'Status', 'Reservations', 'Registrations', 'By state', 'R1 attempts', 'R2 attempts']}>
          {rows.map(({ o, c, r1Attempts, r2Attempts }) => (
            <Tr key={o.id}><Td><Link href={`/admin/opens/${o.id}`} className="link-rule">{o.name}</Link></Td><Td><StatusBadge status={o.status} /></Td><Td mono>{c.reservations}</Td><Td mono>{c.registrations}</Td><Td className="text-[12.5px]">{Object.entries(c.byState).map(([k, v]) => `${k} ${v}`).join(' · ') || '—'}</Td><Td mono>{r1Attempts}</Td><Td mono>{r2Attempts}</Td></Tr>
          ))}
        </Table>
      </Card>
    </>
