...
export default async function OpensPage() {
  ...
  return (
    <Container className="py-16">
      ...
      <div className="mt-12">{live.map((o) => <OpenCard key={o.id} open={o} counts={counts[o.id]} />)}{!live.length && <p className="text-slate">Nothing is open right now.</p>}</div>
      {past.length > 0 && (<><h2 className="plate mt-16">Completed</h2><div className="mt-4">{past.map((o) => <OpenCard key={o.id} open={o} counts={counts[o.id]} />)}</div></>)}
    </Container>
  );
}
