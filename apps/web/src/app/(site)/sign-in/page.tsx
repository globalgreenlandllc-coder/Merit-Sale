...
export default async function SignInPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [mode, personas, site] = await Promise.all([authMode(), demoPersonasEnabled(), siteMode()]); const provider = emailProviderName();
  ...
  return (
    <Container className="grid gap-12 py-16 lg:grid-cols-[1fr_1fr]">
      <div>
        ...
        {sp.switched === 'live' && <Notice className="mt-6" tone="verify" title="The site is live">Demonstration personas are signed out. Sign in with the address listed as a platform admin to return to the console.</Notice>}
        ...
        {mode === 'demo' ? (
          <form action={signInLocal} className="mt-8 max-w-md space-y-4">
            ...
            <p className="text-[12.5px] text-graphite">{site === 'demo' ? 'Demonstration: any address signs in directly as a registrant, no code is sent, and nothing here is a real account.' : 'Development sign-in: any address signs in directly.'}</p>
          </form>
        ) : step === 'email' ? (
        ...
        <p className="mt-6 text-[12.5px] text-graphite">Site: {site} · sign-in: {mode} · delivery: {provider}{mode === 'email' && !staffListsConfigured() ? ' · no staff addresses are listed on this deployment' : ''}</p>
      </div>

      {personas && (
        <div className="rounded-md border hair bg-parchment/60 p-6">
          <div className="flex items-center justify-between"><Plate>Demo personas</Plate><Badge tone="amber">demo mode only</Badge></div>
          <p className="mt-2 text-[13.5px] text-slate">Seeded accounts for each realm, one click each. Anyone who can see this page can use them, which is why they exist only while the site is in demonstration mode.</p>
