...
export async function runSeed(client: PrismaClient, sealKey: string): Promise<Record<string, number>> {
  ...
  const legalDir = [resolve(/* turbopackIgnore: true */ process.cwd(), '../../docs/legal'), resolve(/* turbopackIgnore: true */ process.cwd(), 'docs/legal')].find((d) => existsSync(d));
  const read = (f: string, fallback: string) => (legalDir && existsSync(resolve(legalDir, f)) ? readFileSync(/* turbopackIgnore: true */ resolve(legalDir, f), 'utf8') : fallback);
