...
/**
 * Seed: three Merit Opens at different stages plus a reservation-phase property.
...
import { PrismaClient, type Registration, type Item } from '@prisma/client';
...
import { scoreAttempt, selectAdvancing, r1Pass, normalizeString, type FormItemKey, type ScoringSpec } from '@etk/scoring';
...
const SEAL_KEY = process.env.ADMINISTRATOR_SEAL_KEY ?? '';
if (SEAL_KEY.length !== 64) throw new Error('ADMINISTRATOR_SEAL_KEY (64 hex chars) is required to seed sealed packages');
...
function rulesConfig(o: { slug: string; fee: number; cash: number; states: string[]; openAt: Date; closeAt: Date; rounds: { number: 'r1' | 'r2' | 'r3' | 'final'; windowStart?: Date; windowEnd?: Date; scheduledAt?: Date; durationSeconds: number; integrityTier: 1 | 2 | 3; itemCount: number }[]; N: number; M: number; disclosures?: RulesetConfigInput['disclosures']; practice?: boolean }): RulesetConfigInput {
  return {
    ...
    advanceN: o.N, advanceM: o.M, tieOrderSubsets: { r2: [8, 9, 10], r3: [3, 4] }, r1LatencyGraceSeconds: 3, disputeWindowHours: 72, refundSlaDays: 30, firstAccessHours: 72,
...
async function main() {
  ...
  const legalDir = [resolve(process.cwd(), '../../docs/legal'), resolve(process.cwd(), 'docs/legal')].find((d) => existsSync(d));
  const read = (f: string, fallback: string) => (legalDir && existsSync(resolve(legalDir, f)) ? readFileSync(resolve(legalDir, f), 'utf8') : fallback);
  const rulesText = read('02_Official_Rules_DRAFT.txt', 'OFFICIAL RULES\n[Counsel draft not found in docs/legal]');
  await db.siteSetting.create({ data: { key: 'legal.terms', valueJson: JSON.stringify({ version: '1.0', status: 'draft for counsel', text: read('03_Platform_Terms_of_Service_DRAFT.txt', 'PLATFORM TERMS OF SERVICE\n[Counsel draft not found]') }) } });
  ...
  const practiceProp = await db.property.create({ data: { slug: 'practice-event', name: 'Practice event · no property conveyed', address: 'n/a', city: 'Online', state: 'WA', titleStatus: 'owned', status: 'live', description: 'Practice Merit Opens carry a small cash award so the mechanics can be tested at scale. No property is conveyed.', createdAt: days(-70) } });
  const ps = await db.meritOpen.create({ data: { slug: 'practice-summer', propertyId: practiceProp.id, name: 'The Summer Practice Merit Open', city: 'Online', isPractice: true, stateEligibilityJson: JSON.stringify(ELIGIBLE), registrationFeeCents: 0, cashComponentCents: 250000, registrationOpenAt: days(-60), registrationCloseAt: days(-40), advanceN: 10, advanceM: 3, status: 'complete', closedAt: days(-20), winnerConsentToPublish: true, createdAt: days(-70) } });
  ...
  const lark = await db.property.create({ data: {
    ...
    neighborhood: 'Hollow Creek sits fifteen minutes from the county seat with a K–8 school, a weekly market, and a rail trail reached on foot from the property. [Counsel to approve all factual claims before publication.]',
  ...
  const hc = await db.meritOpen.create({ data: { slug: 'hollow-creek', propertyId: lark.id, name: 'The Hollow Creek Merit Open', city: 'Hollow Creek', stateEligibilityJson: JSON.stringify(ELIGIBLE), registrationFeeCents: 2500, cashComponentCents: 17500000, reservationTarget: 5000, registrationOpenAt: days(-3), registrationCloseAt: days(21), firstAccessHours: 72, advanceN: 2000, advanceM: 100, status: 'registration', createdAt: days(-28) } });
  ...
  const sched = [
    { number: 'r1', sequence: 1, type: 'qualifier', windowStart: days(23), windowEnd: days(25), durationSeconds: 60, integrityTier: 1, formId: hcR1.form.id, reserveFormId: hcR1res.form.id },
    { number: 'r2', sequence: 2, type: 'items', windowStart: days(27), windowEnd: days(29), durationSeconds: 600, integrityTier: 1, capacity: 2000, formId: hcR2.form.id, reserveFormId: hcR2res.form.id },
    { number: 'r3', sequence: 3, type: 'proctored', scheduledAt: days(35), durationSeconds: 5400, integrityTier: 2, capacity: 100, formId: hcR3.form.id, reserveFormId: hcR3res.form.id },
    { number: 'final', sequence: 4, type: 'optimization', scheduledAt: days(42), durationSeconds: 10800, integrityTier: 3, formId: hcF.form.id, reserveFormId: hcFres.form.id },
    { number: 'tiebreak_1', sequence: 5, type: 'tiebreak', scheduledAt: new Date(days(42).getTime() + 4 * 3600e3), durationSeconds: 3600, integrityTier: 3, formId: hcT1.form.id },
    { number: 'tiebreak_2', sequence: 6, type: 'tiebreak', scheduledAt: new Date(days(42).getTime() + 6 * 3600e3), durationSeconds: 3600, integrityTier: 3, formId: hcT2.form.id },
  ];
  for (const r of sched) await db.round.create({ data: { meritOpenId: hc.id, latencyGraceSeconds: 3, ...r } });
  const hcCfg = rulesConfig({ slug: hc.slug, fee: 2500, cash: 17500000, states: ELIGIBLE, openAt: days(-3), closeAt: days(21), N: 2000, M: 100, rounds: [
    { number: 'r1', windowStart: days(23), windowEnd: days(25), durationSeconds: 60, integrityTier: 1, itemCount: 1 }, { number: 'r2', windowStart: days(27), windowEnd: days(29), durationSeconds: 600, integrityTier: 1, itemCount: 10 },
    { number: 'r3', scheduledAt: days(35), durationSeconds: 5400, integrityTier: 2, itemCount: 4 }, { number: 'final', scheduledAt: days(42), durationSeconds: 10800, integrityTier: 3, itemCount: 1 }],
    disclosures: [{ templateKey: 'CA-BP-17539.1 (template; renders when CA is cleared)', maxRounds: 4, maxCostCents: 2500, laterRoundsHarder: true, endDate: days(42).toDateString(), tieMethod: 'Ordered rules only: total score, then a published tie-order subset, then elapsed time (Rounds 2–3). Final ties are resolved by sealed sudden-death problems. No drawing at any stage.', priorEventStats: `Summer Practice Merit Open: 40 registrants, ${r1Pass_.filter(Boolean).length} qualified from Round 1, ${result.advancingCount} advanced from Round 2 (inclusion rule ${result.inclusionRuleApplied ? 'applied' : 'not applied'}).` }] });
  ...
  const alder = await db.property.create({ data: { slug: 'alder-ridge-house', name: 'The Alder Ridge House', address: '[Address published when title records]', city: 'Cedar Hollow', state: 'OR', speEntityName: null, titleStatus: 'under_option', appraisedValueCents: null, beds: 4, baths: 3, sqft: 2680, yearBuilt: 2026, status: 'preview', description: 'Under option. A two-storey home nearing completion on a ridge lot. Details, appraisal, and the recorder link are published when the platform takes title.', createdAt: days(-12) } });
  const ch = await db.meritOpen.create({ data: { slug: 'cedar-hollow', propertyId: alder.id, name: 'The Cedar Hollow Merit Open', city: 'Cedar Hollow', stateEligibilityJson: JSON.stringify(ELIGIBLE), registrationFeeCents: 2500, cashComponentCents: 19500000, reservationTarget: 5000, registrationOpenAt: days(45), registrationCloseAt: days(75), advanceN: 2000, advanceM: 100, status: 'reservation', createdAt: days(-12) } });
